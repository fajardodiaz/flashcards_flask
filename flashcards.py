...
@app.route('/')
def index():
    return render_template('welcome.html',message="This is a simple example",name="Pablo Fajardo", cards=db)
...
def cards_view(card_id):
    try:
        card = db[card_id]
        return render_template("card.html", card=card, card_id=card_id, max_card_id=len(db)-1)
    except IndexError:
        abort(404)
...
def delete_card(card_id):
    try:
        if request.method == 'POST':
            ...
            save_db()
            return redirect(url_for('cards'))
        return render_template('remove_card.html',card= db[card_id])
    except IndexError:
        abort(404)
